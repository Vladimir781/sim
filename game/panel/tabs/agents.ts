...
import type { AgentListRow, FramePayload } from '../../types.d.ts';
...
export function createAgentsTab(ctx: TabContext): TabInstance {
  ...
  const selectAll = element.querySelector('[data-select="all"]') as HTMLInputElement;
  ...
  let currentRows: AgentListRow[] | null = null;
  ...
  function renderRows(rows: AgentListRow[]): void {
    tbody.innerHTML = '';
    currentRows = rows;
    for (const row of rows) {
      ...
      tr.querySelector('input')!.addEventListener('change', (ev) => {
        ...
        if (checked) selection.add(row.id);
