export type EventType = 'storm' | 'raid' | 'drought' | 'ritual';
...
export function createStoryteller(): StorytellerState {
  return { tension: 0.3, budget: 1, cooldown: 0, log: [] };
}

export function stepStoryteller(state: StorytellerState, tick: number): StoryEvent | null {
  if (state.cooldown > 0) {
    state.cooldown--;
    return null;
  }
