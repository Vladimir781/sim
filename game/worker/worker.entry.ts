...
import type { UiToWorkerMessage, WorkerMessage } from '../types.d.ts';
...
function ensureSimulation(): Simulation {
  if (!simulation) {
...
function startLoop(): void {
  ...
  const tick = () => {
    if (running && simulation) {
      let stats: ReturnType<Simulation['step']> | null = null;
...
function emitFrame(stats: ReturnType<Simulation['step']>): void {
  ...
  ctx.postMessage({ type: 'FRAME', payload: frame } satisfies WorkerMessage);
}

function emitTimeline(): void {
  if (!simulation) return;
  const events = simulation.timelineEvents().map((item) => ({ tick: item.tick, description: item.description }));
  ctx.postMessage({ type: 'TIMELINE', payload: events } satisfies WorkerMessage);
}

function emitAgents(): void {
  if (!simulation) return;
  const rows = simulation.agentRows();
  ctx.postMessage({ type: 'AGENT_LIST', payload: { page: 0, total: rows.length, rows } } satisfies WorkerMessage);
}

ctx.addEventListener('message', async (event: MessageEvent<UiToWorkerMessage>) => {
  ...
  switch (message.type) {
    ...
    case 'AGENTS': {
      ...
      if (action === 'kill') sim.killAgents(ids);
      if (action === 'freeze') ids.forEach((id) => sim.freeze(id, true));
      if (action === 'fsm') ids.forEach((id) => sim.switchMode(id, 'fsm'));
      if (action === 'priority') ids.forEach((id) => sim.setThinkEvery(id, 1));
      if (action === 'mute') ids.forEach((id) => sim.toggleMute(id, true));
      emitAgents();
    ...
    case 'SAVE': {
      if (message.payload.action === 'save') {
        ...
        ctx.postMessage({ type: 'SAVE_DONE', payload: { slot: message.payload.slot } } satisfies WorkerMessage);
      } else if (message.payload.action === 'load') {
        const data = await loadSlot(message.payload.slot);
        if (data) {
          simulation = new Simulation();
          ctx.postMessage({ type: 'LOAD_DONE', payload: { slot: message.payload.slot } } satisfies WorkerMessage);
        }
